...
Nous avons également créé notre newsletter pour commencer à récupérer des contacts à activer par email. Nous prévoyons de transformer cette newsletter en *waiting list* une fois notre premier produit disponible en accès beta privée.
...
**Création d'une organisation :** formulaire par étapes pour qu'un utilisateur renseigne les informations nécessaire à la création d'une organisation,
...
**Création de l'API GraphQL :** création d'un serveur wweb en Rust avec des fonctions permettant de lire, traiter et sauvegarder les données dans nos bases de données.
...
**Gestion et centralisation des journeaux d'erreurs :** mise en place du système d'alertes pour simplifier l'identification et le traitement des problèmes techniques que les utilisateurs pourraient rencontrer en utilisant nos produits
...
- [TypeScript](https://www.typescriptlang.org/) pour le frontend avec les librairies:
...
- un serveur créé par Vite qui s'occupe de transpiller notre code TypeScript à chaque modification.

Afin d'éviter les problèmes de CORS, nous avons configurer Vite pour qu'il créé un proxy vers notre serveur back-end.  

Et lorsque nous packagons notre application, le serveur back-end contient les fichiers statiques de notre application frontend et s'occupe de les servir.

Cela nous apporte ces avantages:
- nous n'avons pas besoin de spécifier l'URL du serveur back-end dans notre application frontend
- nous n'avons pas besoin de gérer les CORS côté back-end
- nous avons qu'un seul serveur à gérer en production (donc pas besoin de synchroniser le déploiement de 2 serveurs)

### Focus sur : création de l'API GraphQL

Comme précisé dans la partie précédente, nous avons décidé de créer une API GraphQL pour notre application.
Cela nous apporte des avantages à la fois technique mais aussi sur l'organisation de notre équipe :
- la communication entre le frontend et le back-end est *type-safe* <sup>2</sup>, rendant le développement plus fluide et évite des oublies en cas de modification de l'API
...
Pour le développement de cette API, nous nous sommes tourné vers Rust. Nous avons conscience que c'est un choix particulier car Rust est un langage qui demande de l'expérience et n'a pas une communauté aussi grande que le JavaScript, notamment pour le développement web.  
Mais Rust a un ecosysteme assez grand pour créer des applications web robustes et performantes :
- [Rocket](https://rocket.rs/) permet de créer facilemenent un server web qui combine à la fois le multi-threading et le code asynchrone pour créer des applications performantes,
- l'utilisation de [SQLx](https://github.com/launchbadge/sqlx) nous permet d'écrire des requêtes SQL qui sont validées à la compilation et donc rends le développement plus rapide en nous indiquant les erreurs de syntaxe avant l'exécution. Cela nous permet aussi de nous indiqué toutes les requêtes SQL à modifier en cas de modification modèle de données,
- Et avec [Juniper](https://github.com/graphql-rust/juniper) nous pouvons créer une API GraphQL qui se base sur le system de typage de Rust. Donc toute modification dans le code est automatiquement répercuté sur le schéma GraphQL, schéma qui est ensuite utilisé par notre application frontend pour inférer le typage des requêtes.
...
<sup>1</sup> : abréviation de "Pull Request" en anglais. C'est une fonctionnalité pour proposer et discuter des modifications de code avant de les fusionner avec le reste du projet<br />
