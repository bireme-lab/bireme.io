...
// Screenshot Tooltip + ContextMenu
...
Nous nous sommes tourné vers la librairie [Floating UI](https://floating-ui.com/) qui propose une API à la fois simple et modulaire pour répondre à ces problèmatiques.

Par défaut `Floating UI` va simplement positionner la bulle à côté de l'élément cible.
Mais grâce au système de *middleware*<sup>2</sup>, on peut très simplement rendre le positionnement de la bulle plus intelligente pour résoudre les points cités plus tôt.

Vous pouvez retrouver la liste des *middlewares* fournis [sur la documentation officielle de Floating UI](https://floating-ui.com/docs/middleware).

Dans notre cas, nous avons utilisé les *middlewares* suivant:
- `offset` qui permet d'ajouter un espace entre la bulle et l'élément cible
- `flip` qui permet de changer automatiquement la position de la bulle de gauche à droite, ou de haut en bas s'il manque de l'espace entre la cible et le bord de l'écran
- `shift` qui permet de décentrer la bulle par rapport à l'élément cible au cas où elle sort partiellement du bord de l'écran.

### Focus sur : Workflow Designer => Développeur

Avec nos années d'expériences en tant que développeurs, nous avons pris conscience à quelle point il était important de créer des maquettes pensées dès le début pour leur développement.

Nous avons donc décider de définir les primitives qui constitueront le design de nos application avec les même conventions à la fois dans Figma côté designer et dans le code source de nos application côté développeur:
- la liste des couleurs
- la liste des différentes typographie

Du côté design, on s'assure de garder une cohérence graphique dans l'ensemble de l'application.  
Du côté développement, on facilite l'intégration en utilisant uniquement les variables définies.  

Antoine n'est pas un mec parfait, mais on ne peut pas lui reprocher de vouloir bien faire les choses et s'améliorer (à part pour les blagues de uc mais bon personne n'est parfait)
...
<sup>2</sup> : les middlewares sont des fonctions qui sont appelées les une après les autres pour modifier le comportement d'une librairie
